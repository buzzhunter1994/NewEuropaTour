...
class SortableBehavior extends CActiveRecordBehavior
{
    ...
    public function savePositions($ids, $start)
    {
        ...
        foreach ($ids as $id)
            $priorities[$id] = $start--;
        Yii::app()->db->createCommand("UPDATE " . $this->getOwner()->tableName() . " SET {$this->column} = " . $this->_generateCase($priorities) . " WHERE id IN(" . implode(', ', $ids) . ")")->execute();
    }
    public function prepareTable()
    {
        Yii::app()->db->createCommand("UPDATE " . $this->getOwner()->tableName() . " SET {$this->column} = id")->execute();
    }
