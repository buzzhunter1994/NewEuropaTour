...
$(document).on('click','#{$this->grid->id} a.{$this->class}', function(){
    ...
    return false;
        ...
        $style = "
        .toggle-link .ic-on, .toggle-link .ic-off {display:block; margin:0 auto;}
        .toggle-link .ic-on {display:none;}
        .toggle-link .ic-off {display:block;}
        .toggle-link.toggle-true .ic-on {display:block !important;}
        .toggle-link.toggle-true .ic-off {display:none !important;}
        .ic-on{ color:green;}
        .ic-off{ color:#ccc;}
        ";
    ...
    protected function renderDataCellContent($row,$data)
    {
        ...
        if ($visible)
        {
            ...
            $onImage = "<div class='ic-on' style='$iconStyle' title='{$this->titles[1]}'><i class='fa fa-check'></i></div>";
            $offImage = "<div class='ic-off' style='$iconStyle' title='{$this->titles[0]}'><i class='fa fa-check'></i></div>";
            if (!empty($url))
